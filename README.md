# 🤖 AI Avatar - Agente IA Multientorno UTH
...
Este repositorio corresponde a la **versión específica de UTH**. El proyecto cuenta con una copia hermana para TESCHA, independiente en archivos y assets (logo y modelo 3D propios), pero con el mismo funcionamiento y arquitectura.
...
```bash
git clone <URL-DEL-REPO-UTH>
```

⚠️ Reemplaza `<URL-DEL-REPO-UTH>` por la URL real en cuanto crees el repositorio en GitHub (algo como `https://github.com/tu-usuario/agente-ia-multientorno-uth.git`).

Ingresa al proyecto

```bash
cd agente-ia-multientorno-uth
```
...
Descarga el modelo 3D del avatar (no viene incluido en el repositorio por su tamaño — 118 MB, excede el límite de GitHub) y colócalo en `public/models/avatar/`:

```bash
curl -L -o public/models/avatar/uth_avatar_final_animado.glb <URL-DEL-RELEASE-UTH>/uth_avatar_final_animado.glb
```

⚠️ Reemplaza `<URL-DEL-RELEASE-UTH>` una vez que subas el `.glb` como GitHub Release de este repositorio (mismo procedimiento que se usó para la copia de TESCHA — ver `docs/despliegue-render.md`, sección de GitHub Releases).
...
| `POSTGRES_DB` | Nombre de la base de datos (`uth_tramites`) |
