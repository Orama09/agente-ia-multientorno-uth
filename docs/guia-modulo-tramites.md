...
Documentación técnica del módulo de solicitud y seguimiento de trámites académicos en **AI Avatar** (UTH).

Público objetivo: desarrolladores que mantengan o extiendan este módulo.

> Es un flujo **simulado**, sin conexión al sistema escolar real de UTH — no valida matrícula contra una base de datos institucional, ni genera documentos oficiales firmados.
...
3. createTramite() → INSERT en Postgres (tabla "tramites", estado inicial: "recibido")
        ↓
4. Folio generado (UTH-XXXXXXXX) se muestra al estudiante (con botón copiar)
        ↓
...
7. updateTramiteEstado() → UPDATE en Postgres (estado + fecha_en_proceso o fecha_resuelto)
        ↓
8. 8. Si el nuevo estado es "completado" o "rechazado":
...
| Columna | Tipo | Notas |
|---------|------|-------|
| `folio` | `TEXT` | Primary key. Formato `UTH-XXXXXXXX` (8 caracteres del primer segmento de un UUID v4, en mayúsculas) |
| `nombre` | `TEXT` | |
| `matricula` | `TEXT` | |
...
- Parte de una plantilla ya membretada: `public/documents/templates/hoja_membretada_uth.pdf`, y **dibuja texto encima** (no genera el PDF desde cero).
...
```env
# Postgres
POSTGRES_USER=uth_admin
POSTGRES_PASSWORD=
POSTGRES_DB=uth_tramites
...
# DATABASE_URL=postgresql://uth_admin:tu_password@localhost:5432/uth_tramites
...
APP_BASE_URL=http://localhost:3001   # usado para construir los enlaces del correo
```
...
- Plantilla PDF: `public/documents/templates/hoja_membretada_uth.pdf`