...
Guía para publicar **AI Avatar UTH** en una URL pública gratuita, usando [Render](https://render.com). Pensada para una demo temporal (defensa), no para producción institucional permanente.
...
|----------------------------------------|------------------------|
| `nextjs` | Web Service (Docker, desde `Dockerfile.render`) |
...
2. Nombre: `uth-tramites-db` (o el que prefieras).
3. Plan: **Free**.
...
1. **New +** → **Web Service**.
2. Elige **"Existing Image"** (no un repo de GitHub).
3. Imagen: `chromadb/chroma:latest`.
4. Nombre: `uth-chroma`.
5. Región: la misma que uses para los demás servicios.
6. Plan: **Free**.
7. Una vez creado, copia la **URL pública** que Render le asigna (algo como `https://uth-chroma.onrender.com`) — la necesitarás como `CHROMA_URL` en el paso siguiente.
...
Como `uth_avatar_final_animado.glb` probablemente pese más de 100 MB (el límite de GitHub para archivos normales del repo), **no se sube al repositorio directamente**. Se sube como adjunto de un Release:
...
3. Arrastra `uth_avatar_final_animado.glb` a la zona de adjuntos y publica.
4. Copia el enlace de descarga directa (algo como `https://github.com/<tu-usuario>/<repo-uth>/releases/download/assets-v1/uth_avatar_final_animado.glb`).
...
RUN mkdir -p public/models/avatar && \
    curl -L -o public/models/avatar/uth_avatar_final_animado.glb https://github.com/<tu-usuario>/<repo-uth>/releases/download/assets-v1/uth_avatar_final_animado.glb
...
CHROMA_URL=https://uth-chroma.onrender.com
...
SENDGRID_TO_EMAIL=
ADMIN_KEY=
APP_BASE_URL=https://uth-agente-ia-multientorno.onrender.com
```
...
- [ ] Repositorio de UTH creado en GitHub y código subido
- [ ] `.glb` de UTH subido como GitHub Release, enlace de descarga confirmado
