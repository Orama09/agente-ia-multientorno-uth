...
function getResolutionMessage(tramite: Tramite): string {
  ...
  if (esReporte) {
    return tramite.estado === "completado"
      ...
      : "No fue posible darle seguimiento a tu reporte con la información proporcionada. Te invitamos a generar uno nuevo con más detalle, o acudir de manera presencial a las oficinas del UTH.";
  }

  return tramite.estado === "completado"
    ? `Puedes recoger tu documento en las oficinas del UTH dentro de los próximos 2 días hábiles. ${HORARIO_CONTROL_ESCOLAR}`
    : `Tu solicitud no pudo ser procesada. Puedes generar una nueva en un plazo de 2 días, o acudir de manera presencial a las oficinas del UTH. ${HORARIO_CONTROL_ESCOLAR}`;
}
