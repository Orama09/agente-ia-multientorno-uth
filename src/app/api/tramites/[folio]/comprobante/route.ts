...
// Honduras (San Pedro Sula / Tegucigalpa) usa UTC-6 todo el año, sin horario de verano.
// America/Mexico_City NO es correcto para Honduras (además, CDMX cambió de zona en 2022).
const TZ = "America/Tegucigalpa";

function formatFechaHora(iso: string): string {
  return new Date(iso).toLocaleString("es-HN", { timeZone: TZ });
}

function formatFecha(iso: string): string {
  return new Date(iso).toLocaleDateString("es-HN", { timeZone: TZ });
}
...
export async function GET(
  ...
  const templateBytes = await fs.readFile(
    path.join(process.cwd(), "public", "documents", "templates", "hoja_membretada_uth.pdf")
  );
  ...
  // --- TIPO: celda real va de x=61 (fin de "Tipo:") a x=217 (borde derecho) ---
  const tipoX = 70;
  const tipoMaxWidth = 142; // 217.1 - 70 - margen de seguridad
  let tipoSize = 10;
  while (
    bold.widthOfTextAtSize(tipoLabel, tipoSize) > tipoMaxWidth &&
    tipoSize > 6
  ) {
    tipoSize -= 0.5;
  }
  ...
  // --- FECHA: celda real va de x=262 (fin de "Fecha:") a x=358 (borde derecho) ---
  const fechaX = 267;
